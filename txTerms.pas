  ...
  if FEnabled then
    if Use_ObjPath then
      ...
      while (x<>0) and (Result=0) do
       begin
        if DBExists('SELECT Tok.id FROM Tok LEFT JOIN TokType ON TokType.id=Tok.toktype_id WHERE Tok.obj_id=? AND TokType.system=''wiki.prefix'' LIMIT 1',[x]) then
          Result:=x
        else
...
begin
  ...
  Result:=DBExists('SELECT Trm.obj_id FROM Trm WHERE Trm.domain_id=? AND lower(Trm.term)=lower(?) LIMIT 1',[FStore.FCurrentDomain,Name]);
